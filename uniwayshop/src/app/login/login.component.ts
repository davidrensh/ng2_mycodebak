...
export class LoginComponent implements OnInit, OnDestroy {
  ...
  afterLogin() {
    ...
    this.sub1 = this.af.object('/employees/' + this.ss.emailNode(this.phoneemail)).valueChanges().subscribe((o) => {
      if (o && !o['deleted']) {
    ...
    this.sub3 = this.af.object('/c').valueChanges().subscribe((o) => {
      this.ss.setStore(o);
    });
  }
  failedLogin() {
  ...
  login() {
    if (this.isEmail()) {
      ...
      this.sub2 = this.af.object('/p/' + this.ss.phoneNode(this.phoneemail)).valueChanges().subscribe((o) => {
        if (o) {
