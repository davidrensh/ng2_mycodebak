...
export class ReportsComponent implements OnInit {
  ...
  showYearRpt() {
    ...
    const dataO = this.af.list('/d', r =>
      r.startAt(d.valueOf().toString()).endAt(dEnd.valueOf().toString()).orderByKey()
    );
  dataO.valueChanges().subscribe(p => {
    // this.dataAll = p;
...
showMonthRpt() {
  ...
  // console.log(dEnd);
  // console.log('yeard', d, d.valueOf().toString());
  const dataO = this.af.list('/d', r => r.orderByKey()
...
getRptSubType(o) {
  ...
  switch (o.viewValue) {
    case 'Employee':
      ...
      const sube = es.snapshotChanges().subscribe(s => {
        s.filter((e) => {
    ...
    case 'Location':
      ...
      const sub = items.valueChanges().subscribe(s => {
        s.forEach(e => {
