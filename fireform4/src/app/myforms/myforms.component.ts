...
export class MyformsComponent implements AfterViewInit, OnChanges, OnDestroy, OnInit {
  ...
  public tb: DynamicTypeBuilder;

  public selectedform = '';
  constructor(public rs: RoleService, public typeBuilder: DynamicTypeBuilder,
  ...
  selectFormWithName(name: string) {
    this.signed = false;
    this.ssHtml = this.af.object('/forms/' + name).subscribe(res => {
      if (res && res.contenthtml) {
        ...
        const convertedHtml: string = this.ConvertToNg2Template(res.contenthtml);
        this.html = convertedHtml;
        if (this.html !== undefined && this.html.length > 0) {
          this.ssVar = this.af.list("/forms/" + this.selectedName + "/data").subscribe(items => {
            items.map(item => {
  ...
  ngOnInit() {
    ...
    this.sub = this.route.params.subscribe(params => {
      this.selectedName = params['id'];
      this.fromNav = false;
      this.ssHtml = this.af.object("/forms/" + this.selectedName).subscribe(res => {
        if (res && res.contenthtml) {
          this.attrdata["typename"] = res.typename;
          this.attrdata["persontypename"] = res.persontypename;
          this.attrdata["password"] = res.password;
    ...
    while ((match = s.indexOf(toMatch, i)) > -1) {
      ...
      if (matchEnd > -1) {
        ...
        if (exname.indexOf("'") < 0 && exname.indexOf("\"") < 0) {
          ...
          let sVar = this.af.list("/forms/" + exname + "/data").subscribe(items => {
  ...
  saveData() {
    ...
    if (MyformsComponent.namelist !== undefined) {
      for (var i = 0; i < MyformsComponent.namelist.length; i++) {
        let n = MyformsComponent.namelist[i];
        // console.log("dataall:" + JSON.stringify(this.data) + "name:" + n + "datan=" + this.data[n] + "txtArea:" + this.data["txtArea"]);

        this.af.object("/forms/" + this.selectedName + "/data/" + n).update({
          varname: n,
  ...
  ReplaceWithParam(src: string, tag: string, typename: string): string {
    ...
    if (typename === "") {
      ...
      var p = src.replace(re, function (match, a, b, c, d, e) {
        ...
        if (MyformsComponent.namelist.indexOf(d) < 0) MyformsComponent.namelist.push(d);
    ...
    var p = src.replace(re, function (match, a, b, c, d, e, f, g, h) {
      ...
      if (s !== "") {
        ...
        let srep = s.replace(sre, function (match2, p0, p1, q1, r1) {
          ...
          if (MyformsComponent.namelist.indexOf(q1) < 0) MyformsComponent.namelist.push(q1);
  ...
  ConvertRepeator(src: string): string {
    ...
    while (iRepeator !== undefined && iRepeator != -1) {
      ...
      if (datasource !== null && datasource !== "" && repeator !== "") {
        ...
        // this.data["'" + datasource + "." + repeator + "'"] = this.af.list("/forms/" + datasource + "/data/block/" + repeator);
        this.dstable["'" + datasource + "." + repeator + "'"] = datasource + "." + repeator;// this.af.list("/forms/" + datasource + "/data/block/" + repeator);
        ...
        // this.repeatorData[datasource] = this.af.list("/forms/" + datasource + "/data/block/" + repeator);
        // this.repeatorData[datasource] = null;
      } else if (repeator !== "") {
        ds = "data['" + repeator + "']";
        this.dstable["'" + datasource + "." + repeator + "'"] = this.af.list("/forms/" + this.selectedName + "/data/block/" + repeator);
        // this.data["'" + repeator + "'"] = this.af.list("/forms/" + this.selectedName + "/data/block/" + repeator);
  ...
  ConvertTable(src: string): string {
    var p2 = src.replace(/(input.+)(name=")(.+?)(".+)(type="text")(.+)/g, function (match, prefix, handler, name, suffix, suffix2, suffix3) {
      if (MyformsComponent.namelist.indexOf(name) < 0) MyformsComponent.namelist.push(name);
  ...
  wrapTemplate(t: string): string {
    const s = `
  <input *ngIf="staticneedpassword()" [(ngModel)]="enteredPassword" placeholder="Form password">
  <input *ngIf="statictypename() === 'PerPerson' && staticpersontypename() === 'Email' " [(ngModel)]="enteredEmail" placeholder="Email">
  <input *ngIf="statictypename() === 'PerPerson' && staticpersontypename() === 'Phone' " [(ngModel)]="enteredPhone" placeholder="Phone">
<div *ngIf="validPassword()">
    ...
    <button *ngIf="!staticneedsignature() || (staticneedsignature() && signed)"
     class="btn btn-primary-outline btn-sm" (click)="saveData(selectedName)">Save Data</button>
<div  id="print-section"><div class="printborder">` + t + `
  ...
  public refreshContent() {
    ...
    const template = this.wrapTemplate(this.html);

    this.tb.createComponentFactory(template, this.dstable, this.attrdata, MyformsComponent.namelist, this.selectedName)
  ...
  ngAfterViewInit() {
    //console.log('on after view init' + this.dynamicComponentTarget);
    // this returns null
  }
