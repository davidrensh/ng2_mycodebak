...
export class LoginComponent implements OnInit, OnDestroy {
  ...
  loadNumByLastReadDate(r) {
    ...
    news.subscribe(p => {
      const p0 = p.filter(q => {
        ...
        return q.city === this.ss.member.c && res;
      });
      this.ss.setNewsNum(p0.length);
    })
    ...
    flyers.subscribe(p => {
      const p0 = p.filter(q => {
        return q.city === this.ss.member.c && this.categoryChecked(this.ss.categories, q.category);
      });
      this.ss.setFlyersNum(p0.length);
    })
  }
  ...
  loadLoginAndGoToHist(e) {
    this.af.list('/u/' + this.ss.emailNode(e) + '/cats').subscribe(p => {
      this.ss.setMemberCategory(p);
    });
